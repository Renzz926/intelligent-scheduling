...
import { isEmptyArray, getDprSize } from '@/utils/utils';
...
const FallTable = React.memo(
  ({
    ...
    const handleScroll = (arr) => {
      ...
      if (arr.length > Number(defaultSize) && scrollTable) {
        timeRef.current = setInterval(() => {
          container.scrollTop += getDprSize(rollTop);
          if (
            Math.ceil(container.scrollTop) >=
            Number(container.scrollHeight - container.clientHeight) - getDprSize(1)
          ) {
